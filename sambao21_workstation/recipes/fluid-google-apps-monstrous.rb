...
apptitle = "#{node['sprout']['fluid']['instances']['google-apps-monstrous']['apptitle']}"
basename = "#{node['sprout']['fluid']['instances']['google-apps-monstrous']['basename']}"
...
unless File.exists?("/Applications/#{app_path}")
  ...
  execute "copy fluid app to /Applications" do
    command "mv #{cache_path}/#{Regexp.escape("#{app_path}")} #{Regexp.escape("/Applications/#{app_path}")}"
    user node['current_user']
    group "admin"
  end
end
